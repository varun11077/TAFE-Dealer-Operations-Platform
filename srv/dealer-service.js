...
module.exports = cds.service.impl(async function () {
    ...
    this.before("CREATE", "Dealers", async (req) => {
        ...
        if (gstExists) {
            ...
            return req.reject(409, "Dealer with this GST number already exists.");
        }
        ...
        if (panExists) {
            ...
            return req.reject(409, "Dealer with this PAN number already exists."
            );
        }
        ...
        if (phoneExists) {
            ...
            return req.reject( 409, "Dealer with this phone number already exists." );
        }
        ...
        if (email) {
            ...
            if (emailExists) {
                return req.reject(  409,   "Dealer with this email already exists." );
            }
        }
    ...
    this.before("CREATE", "Dealers", async (req) => {
        ...
        * Local development:
        ...
        if (cds.db.kind === "hana") {
            ...
            req.data.dealerCode = `DLR${String(nextValue).padStart(5, "0")}`;
        }
    });
    ...
    this.before("UPDATE", "Dealers", async (req) => {
        ...
        if (req.data.dealerCode !== undefined) {
            ...
            return req.reject(400, "Dealer code cannot be modified.");
        }
        ...
        if (req.data.status !== undefined) {
            ...
            return req.reject(400, "Dealer status can only be changed through business actions.");
        }
        ...
        if (req.data.panNumber) {
            ...
            const existingPAN = await SELECT.one
                .from(Dealer)
                .where({ panNumber: req.data.panNumber });
    ...
    this.on("submitDealer", "Dealers", async (req) => {
        ...
        if (dealer.status !== "PENDING") {
            return req.reject(400, "Only pending dealers can be submitted.");
        }
    ...
    this.on("approveDealer", "Dealers", async (req) => {
        ...
        if (!req.user.is("DealerManager") && !req.user.is("Admin")) {

            return req.reject(403, "You are not authorized to approve dealers.");
        }
        ...
        if (dealer.status !== "PENDING" && dealer.status !== "SUBMITTED") {

            return req.reject(400, "Only pending or submitted dealers can be approved.");
        }
    ...
    this.on("rejectDealer", "Dealers", async (req) => {
        ...
        if (!req.user.is("DealerManager") && !req.user.is("Admin")) {
            ...
            return req.reject(403, "You are not authorized to reject dealers.");
        }
        ...
        if (dealer.status !== "PENDING" && dealer.status !== "SUBMITTED") {
            ...
            return req.reject(400, "Only pending or submitted dealers can be rejected.");
        }


        await UPDATE(Dealer).set({
            status: "REJECTED",
            remarks: reason.trim()
        })
            .where({ ID: dealerId });
    ...
    this.on("blockDealer", "Dealers", async (req) => {
        ...
        if (!req.user.is("DealerManager") && !req.user.is("Admin")) {
            ...
            return req.reject(403, "You are not authorized to block dealers.");
        }
        ...
        if (!reason || !reason.trim()) {
            ...
            return req.reject(400, "Block reason is mandatory.");
        }
        ...
        if (dealer.status !== "ACTIVE") {
            ...
            return req.reject(400, "Only active dealers can be blocked.");
        }
    ...
    this.before("DELETE", "Dealers", async (req) => {
        ...
        return req.reject(405, "Dealer deletion is not allowed. Use business status actions.");
    }
    );
